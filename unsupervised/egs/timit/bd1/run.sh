...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # make json labels
    ./local/data2json.sh --feat ${feat_tr_dir}/feats.scp --trans_type ${trans_type} \
    data/${train_set} ${dict} > ${feat_tr_dir}/data.json
    ./local/data2json.sh --feat ${feat_dt_dir}/feats.scp --trans_type ${trans_type} \
    data/${train_dev} ${dict} > ${feat_dt_dir}/data.json
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        ./local/data2json.sh --feat ${feat_recog_dir}/feats.scp --trans_type ${trans_type} \
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    nj=1
    for weight in $(seq 0 0.5 1); do
        for rtask in ${recog_set}; do
            (
                decode_dir=decode_${rtask}_$(basename ${decode_config%.*})_boundaries_w${weight}
                feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
                ...
                --result-label ${expdir}/${decode_dir}/ \
