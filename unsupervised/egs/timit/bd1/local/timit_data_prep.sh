...
tmpdir=$(mktemp -d ${dir}/kaldi.XXXX);
...
for x in train dev test; do
    ...
    find $1/{$train_dir,$test_dir} -not \( -iname 'SA*' \) -not \( -iname '*.WAV.wav' \)  -iname '*.WAV' \
    ...
    then
        ...
        ${local}/timit_format_phn_frames.py $tmpdir/${x}_phn.flist > $tmpdir/${x}_phn.frames
        cat $tmpdir/${x}_phn.frames | sort -k1,1 > ${x}.frames
        while read line; do
