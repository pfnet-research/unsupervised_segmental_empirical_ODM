...
# 3.1 Create phoneme file
if [ -n "${phndic}" ]; then
    cat ${dir}/phn > ${tmpdir}/output/phone.scp
    < ${tmpdir}/output/phone.scp utils/sym2int.pl --map-oov ${phnoov} -f 2- ${phndic} > ${tmpdir}/output/phoneid.scp
fi
