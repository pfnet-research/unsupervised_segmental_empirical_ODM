...
def train(args):
    ...
    # Setup a converter
    converter = model.CustomConverter(args.layer_type)
    ...
    load_tr = LoadInputsAndTargets(
        mode='bound', load_output=True, preprocess_conf=args.preprocess_conf,
    ...
    if ngpu <= 1:
        ...
        if args.n_iter_processes > 0:
            train_iters = [ToggleableShufflingMultiprocessIterator(
                ...
                batch_size=1, n_processes=args.n_iter_processes, n_prefetch=15, maxtasksperchild=20,
                shuffle=not use_sortagrad)
                for gid in six.moves.xrange(ngpu)]
        else:
            train_iters = [ToggleableShufflingSerialIterator(
...
def recog(args):
    ...
    with chainer.no_backprop_mode():
        for idx, name in enumerate(js.keys(), 1):
            logging.info('(%d/%d) obtaining boundaries for ' + name, idx, len(js.keys()))
            batch = [(name, js[name])]
            feat = load_inputs_and_targets(batch)[0][0]
            probs, boundaries = model.get_boundary(feat, args.boundary_threshold, args)
            this_file = os.path.join(args.result_label, f'{name}')
            logging.info(f'Saving boundaries with prefix {this_file}')
            if probs is not None:
                np.save(f'{this_file}_pbs', probs)
