...
def train(args):
    ...
    if not chainer.cuda.available:  # noqa pylint: disable=no-member
        logging.warning('cuda is not available')
    if not chainer.cuda.cudnn_enabled:  # noqa pylint: disable=no-member
    ...
    if ngpu == 1:
        ...
        chainer.cuda.get_device_from_id(gpu_id).use()  # noqa pylint: disable=no-member
    ...
    # Setup a converter
    converter = model.CustomConverter(folder=os.path.dirname(args.train_json))
    ...
    load_tr = LoadInputsAndTargets(
        mode='uasr', load_output=True, preprocess_conf=args.preprocess_conf,
    ...
    if ngpu <= 1:
        ...
        train_subsets = [make_batchset(train_json, args.batch_size,
        ...
        if args.n_iter_processes > 0:
            train_iters = [ToggleableShufflingMultiprocessIterator(
                TransformDataset(train_subsets[0], load_tr),
        ...
        # set up updater
        updater = model.CustomParallelUpdater(
    ...
    if args.opt == 'noam':
        from espnet.nets.chainer_backend.training import VaswaniRule
        trainer.extend(VaswaniRule('alpha', d=args.noam_dims, warmup_steps=args.noam_warmup_steps,
                                   scale=args.noam_lr), trigger=(1, 'iteration'))
    # Resume from a snapshot
    if args.resume:
        chainer.serializers.load_npz(args.resume, model)  # trainer)
    ...
    if args.n_iter_processes > 0:
        valid_iter = chainer.iterators.MultiprocessIterator(
            TransformDataset(valid[0], load_cv),
    ...
    valid_converter = model.CustomConverter(folder=os.path.dirname(args.valid_json))
    trainer.extend(BaseEvaluator(
        valid_iter, model, converter=valid_converter, device=gpu_id))
    ...
    trainer.extend(extensions.PlotReport(['main/loss_inter', 'validation/main/loss_inter'],
                                         'epoch', file_name='inter.png'))
    trainer.extend(extensions.PlotReport(['main/loss_intra', 'validation/main/loss_intra'],
                                         'epoch', file_name='intra.png'))
...
def recog(args):
    ...
    model = model_class(idim, odim, train_args, load_lm=False)
    assert isinstance(model, ASRInterface)
    ...
    with chainer.no_backprop_mode():
        for idx, name in enumerate(js.keys(), 1):
            ...
            hyp_idx = nbest_hyps[0]['yseq']
            hyp_phn = [train_args.char_list[i] for i in hyp_idx]
            ref_phn = js[name]['output'][0]['phone'].split(' ')
            
            phn_eds.append(editdistance.eval(hyp_phn, ref_phn))
            phn_ref_lens.append(len(ref_phn))
            new_js[name] = {'output': [{'name': 'target1',
                                        'token': ' '.join(ref_phn), 
                                        'rec_token': ' '.join(hyp_phn),
                                        'rec_tokenid': ' '.join(map(str, hyp_idx))}],
                            'utt2spk': js[name]['utt2spk']}
    per = float(sum(phn_eds)) / sum(phn_ref_lens)

    with open(args.result_label, 'wb') as f:
        f.write(json.dumps({'utts': new_js}, indent=4, ensure_ascii=False, sort_keys=True).encode('utf_8'))
    
    logging.info(f'PER: {per}')
