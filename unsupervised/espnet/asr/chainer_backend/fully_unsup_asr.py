...
def train(args):
    ...
    # Setup a converter
    converter = model.CustomConverter()
    ...
    if args.opt == 'noam':
        ...
        trainer.extend(VaswaniRule('alpha', d=args.noam_dims, warmup_steps=args.noam_steps,
                                   scale=args.noam_scale), trigger=(1, 'iteration'))
    ...
    converter = model.CustomConverter(labels=True)
    ...
    elif args.opt == 'noam' or args.opt == 'adam':
        trainer.extend(extensions.observe_value(
...
def boundaries(args):
    ...
    os.makedirs(args.result_label, exist_ok=True)
    import numpy as np
    # decode each utterance
    with chainer.no_backprop_mode():
        for idx, name in enumerate(js.keys(), 1):
            ...
            boundaries = model.get_boundaries(feat)
            this_file = os.path.join(args.result_label, f'{name}')
            logging.info(f'Saving boundaries with prefix {this_file}')
            np.save(f'{this_file}_lbl', boundaries)
...
def recog(args):
    ...
    with chainer.no_backprop_mode():
        for idx, name in enumerate(js.keys(), 1):
            ...
            batch = [(name, js[name])]
            feat = load_inputs_and_targets(batch)[0][0]
            nbest_hyps = model.recognize(feat, args, train_args.phn_list)
            hyp_idx = nbest_hyps[0]['yseq']
            hyp_phn = [train_args.phn_list[i] for i in hyp_idx]
