...
v2: Experimental API. It supports any models that implements ScorerInterface.''')
    ...
    # unsupervised
    parser.add_argument('--fully-unsupervised', type=strtobool,
                        default=False, help='')
    parser.add_argument('--fully-unsup-boundaries', type=strtobool,
                        default=False, help='')
    parser.add_argument('--unsupervised', type=strtobool,
                        default=False, help='')
    parser.add_argument('--boundary', type=strtobool,
                        default=False, help='')
    parser.add_argument('--boundary-weight', type=float,
                        default=1.0, help='')
    parser.add_argument('--boundary-type', type=str,
                        default='up', help='')
    parser.add_argument('--boundary-threshold', type=float,
                        default=0.005, help='')
    # pretrain
    parser.add_argument('--pretrain', type=strtobool,
                        default=False, help='')
    return parser
...
def main(args):
    ...
    if args.fully_unsupervised:
        logging.info('Fully Unsupervised')
        from espnet.asr.chainer_backend.fully_unsup_asr import recog
    elif args.fully_unsup_boundaries:
        logging.info('Fully Unsupervised Boundaries')
        from espnet.asr.chainer_backend.fully_unsup_asr import boundaries as recog
    elif args.unsupervised:
        logging.info('Unsupervised')
        from espnet.asr.chainer_backend.u_asr import recog
    elif args.boundary:
        logging.info('Boundary')
        from espnet.asr.chainer_backend.boundary import recog
    elif args.pretrain:
        logging.info('Pretrain')
        from espnet.asr.chainer_backend.pre_asr import recog
    else:
        if args.num_spkrs == 1:
