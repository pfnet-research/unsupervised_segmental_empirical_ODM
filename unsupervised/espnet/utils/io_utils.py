...
class LoadInputsAndTargets(object):
    ...
    def __init__(self, mode='asr',
        ...
        self._loaders = {}
        if mode not in ['asr', 'tts', 'mt', 'uasr', 'bound']:
            raise ValueError(
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                if self.mode != 'bound':
                    ...
                    x = np.fromiter(map(int, info['output'][0]['frames'].split()),
                                            dtype=np.int64)
                    y_feats_dict.setdefault(info['output'][0]['name'], []).append(x)

        if self.mode == 'asr' or self.mode == 'bound':
